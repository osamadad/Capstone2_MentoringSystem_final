...
import com.tuwaiq.capstone2_mentoringsystem.Service.GeminiService;
...
public class UserController {
    ...
    public ResponseEntity<?> sendWhatsappMessages(@PathVariable String to,@PathVariable String message){
    ...
    @GetMapping("/ask-gemini/{prompt}")
    public ResponseEntity<?> askGemini(@PathVariable String prompt){
        return ResponseEntity.status(200).body(new ApiResponse(geminiService.askGemini(prompt)));
    }

    @GetMapping("/get-user-info/{userId}")
    public ResponseEntity<?> getUserInfoByUserId(@PathVariable Integer userId) {
        UserProfile userProfile = userService.getUserInfo(userId);
        if (userProfile == null) {
            return ResponseEntity.status(400).body(new ApiResponse("There are no users with this id found"));
        } else {
